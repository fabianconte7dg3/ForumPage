...
export async function crearTutoria(input: CrearTutoriaInput) {
  const usuario = await sesionActual()
  if (!usuario || (usuario.rol !== 'staff' && usuario.rol !== 'admin')) {
    return { error: 'No autorizado para agendar tutorías.' }
  }
  ...
  try {
    ...
    console.error('Error al agendar tutoría:', error)
    return { error: 'Ocurrió un error al agendar la tutoría.' }
  }
}
