...
> **Última actualización:** 2026-08-10 — Auditoría de cobertura contra el Informe Anual 2025 impreso de la Fundación (8 fotos reales aportadas por el usuario): de 9 programas del informe, solo Becas Universitarias está bien cubierto por el sistema; Tutorías era solo un calendario de anuncios; Becas de Comunidad (K-12), Cursos, Talleres, Giras Educativas y Donaciones no tienen colección alguna. Primer gap cerrado a pedido del usuario: `Tutorias` ahora registra `realizada`/`participantes` reales, no solo el anuncio. Verificado con Postgres real: marcar y desmarcar "Se realizó" guarda/limpia correctamente. Detalle completo en [docs/plan.md](file:///home/fabianc/Documentos/ForumPage/docs/plan.md).
> **Actualización anterior (2026-08-09, 2):** Segundo bug real del mismo día, reportado por el usuario: cambiar el idioma del sitio no traducía las publicaciones. Causa raíz sistémica: ninguna de las 21 Server Actions de `/staff` que escriben campos localizados pasaba `locale` a Payload, así que todo se guardaba siempre en español sin importar el idioma del panel — afecta Actividades, Equipo, Recursos, Proyectos, Sedes, Programas, Centros Educativos, Comunidades, Prácticas, Nosotros y Becarios. Corregido en los 21 archivos, con el tipo `Locale` real en vez de `string` + cast. Verificado creando una publicación en español y traduciéndola de verdad desde `/en/staff`: `?locale=es` y `?locale=en` ahora devuelven textos distintos.
> **Última actualización anterior (1):** 2026-08-09 — Bug real reportado por el usuario probando en producción: la galería de fotos de una Publicación nunca se guardaba. Causa raíz encontrada (cierre obsoleto sobre `e.target.files` en el input de galería de `FormularioActividadModal.tsx`, vaciado por `e.target.value = ''` antes de que React leyera el archivo) y corregida, más `bodySizeLimit` de Server Actions subido de 1MB a 20MB como defensivo. Verificado contra una instancia temporal conectada a la base de producción real (nunca contra el sitio público), con 12 fotos reales de celular.
