...
> **Última actualización:** 2026-08-10 (4) — Nueva pestaña "Importar" en `/staff`: el staff puede descargar una plantilla Excel por colección (Tutorías, Cursos, Talleres, Giras Educativas, Donaciones), llenarla con historial real de papel/Excel (con la fecha real de cuando ocurrió cada cosa) y subirla — filas inválidas se reportan una por una sin bloquear las válidas. Becarios históricos queda fuera de esta fase a propósito (consentimiento + cuentas de usuario, diseño aparte). Verificado con login real de staff en el navegador contra Postgres local: 2 de 3 filas de prueba importadas, fecha DD/MM parseada correctamente (no invertida como haría `new Date` nativo), fila inválida rechazada con el motivo exacto. Sin migración. Detalle completo en [docs/plan.md](file:///home/fabianc/Documentos/ForumPage/docs/plan.md).
> **Actualización anterior (2026-08-10, 3):** `/impacto` (pestaña "Resumen") ahora muestra las cifras reales de los 4 programas cerrados en la actualización anterior: tutorías/cursos/talleres/giras realizados (con participantes reales) y donaciones entregadas — antes solo estaban disponibles vía API. Se corrigió en el camino un bug de RSC (función pasada de Server a Client Component) antes de commitear. Verificado con Postgres real vía HTML servido, en ambos idiomas. Sin migración. Detalle completo en [docs/plan.md](file:///home/fabianc/Documentos/ForumPage/docs/plan.md).
> **Actualización anterior (2026-08-10, 2):** Cerrados los 4 gaps restantes de la auditoría del Informe Anual 2025 impreso (ver actualización anterior): Becas de Comunidad K-12 (`Becarios.nivel_educativo`/`tipo_apoyo`, condicionando universidad/carrera al track universitario) + 4 colecciones nuevas (Cursos, Talleres, Giras Educativas, Donaciones) con el mismo patrón `realizada`/`participantes` de Tutorías, agrupadas en una pestaña nueva "Programas" en `/staff`. Verificado con Postgres real: los 5 flujos (becario K-12 + 4 registros nuevos) guardados y confirmados vía API. Migración probada `up`/`down`/`up` completo. Detalle completo en [docs/plan.md](file:///home/fabianc/Documentos/ForumPage/docs/plan.md).
> **Actualización anterior (2026-08-10, 1):** Auditoría de cobertura contra ese mismo informe (8 fotos reales aportadas por el usuario): de 9 programas, solo Becas Universitarias estaba bien cubierto; Tutorías era solo un calendario de anuncios. Primer gap cerrado: `Tutorias` ahora registra `realizada`/`participantes` reales.
> **Anteriores (2026-08-09):** dos bugs reales reportados por el usuario probando en producción — (1) galería de fotos de Publicaciones nunca se guardaba (cierre obsoleto sobre `e.target.files`), (2) el selector de idioma no traducía nada (ninguna Server Action de `/staff` pasaba `locale` a Payload, corregido en 21 archivos). Ambos verificados end-to-end contra datos/instancias reales. Historial completo previo a esa fecha en [docs/plan.md](file:///home/fabianc/Documentos/ForumPage/docs/plan.md).
